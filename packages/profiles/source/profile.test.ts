...
it("should have a default theme", () => {
	expect(subject.appearance().get("theme")).toBe("light");
});

it("should have a custom theme", () => {
	subject.settings().set(ProfileSetting.Theme, "dark");

	expect(subject.appearance().get("theme")).toBe("dark");
});
...
it("should have a exchange transactions repository", () => {
	expect(subject.exchangeTransactions()).toBeInstanceOf(ExchangeTransactionRepository);
});
...
it("should flush all data", () => {
	...
	expect(subject.settings().keys()).toHaveLength(17);
});
