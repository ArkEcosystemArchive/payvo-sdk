...
// 		Mockery.stub(subject.coin().address(), "fromMnemonic").mockImplementation(async () => ({
...
// 				type,
// 			}));
// 			Mockery.stub(subject.coin().extendedPublicKey(), "fromMnemonic").mockImplementation(
// 				async () =>
...
test("#removeEncryption - should remove the encryption password of a wallet imported by mnemonic", async () => {
	...
	Mockery.stub(subject, "address").mockReturnValueOnce(address);
	Mockery.stub(subject, "isSecondSignature").mockReturnValueOnce(false);
...
test("#removeEncryption - should remove the encryption password of a wallet imported by mnemonic with second signature", async () => {
	...
	Mockery.stub(subject, "address").mockReturnValueOnce(address);
	Mockery.stub(subject, "isSecondSignature").mockReturnValueOnce(true);
