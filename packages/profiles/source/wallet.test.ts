...
it("should have an import method", () => {
	expect(subject.importMethod()).toEqual(WalletImportMethod.BIP39.MNEMONIC);
});

it("should have a derivation method", () => {
	expect(subject.derivationMethod()).toEqual("bip39");
});
...
it("should have a ledger model", () => {
	...
	expect(subject.data().get(WalletData.LedgerModel)).toBe(WalletLedgerModel.NanoS);
	expect(subject.isLedgerNanoS()).toBe(true);
	expect(subject.isLedgerNanoX()).toBe(false);

	subject.data().set(WalletData.LedgerModel, WalletLedgerModel.NanoX);
	expect(subject.data().get(WalletData.LedgerModel)).toBe(WalletLedgerModel.NanoX);
	expect(subject.isLedgerNanoX()).toBe(true);
	expect(subject.isLedgerNanoS()).toBe(false);
});
...
it("should have a signing key instance", () => {
	expect(subject.signingKey()).toBeInstanceOf(WalletImportFormat);
});

it("should have a confirmation key instance", () => {
	expect(subject.confirmKey()).toBeInstanceOf(WalletImportFormat);
});

it("should determine if wallet is is a cold wallet", async () => {
	expect(subject.isCold()).toBeBoolean();
});
...
it("should unset cold wallet status if outgoing transaction is found", async () => {
	...
	expect(subject.isCold()).toBeTrue();

	await subject.transactionIndex().all();

	expect(subject.isCold()).toBeFalse();
});
...
it("should have a signatory factory", () => {
	expect(subject.signatoryFactory()).toBeInstanceOf(SignatoryFactory);
});
