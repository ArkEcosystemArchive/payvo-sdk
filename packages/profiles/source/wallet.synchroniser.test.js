...
describe("WalletSynchroniser", ({
	...
	it("should sync the identity with a public key", async () => {
		...
		stub(wallet, "actsWithPublicKey").returnValue(true);
		stub(wallet.network(), "usesExtendedPublicKey").returnValue(false);
	...
	it("should sync the identity with an extended public key", async () => {
		actsWithMnemonic.returnValue(false);
		stub(wallet, "actsWithPublicKey").returnValue(true);
		stub(wallet.network(), "usesExtendedPublicKey").returnValue(true);
	...
	it("should fail to sync the identity with an unknown import method", async () => {
		stub(wallet, "actsWithAddress").returnValue(false);
		actsWithMnemonic.returnValue(false);
		stub(wallet, "actsWithPublicKey").returnValue(false);
		stub(wallet, "actsWithMnemonic").returnValue(false);
		stub(wallet, "actsWithPrivateKey").returnValue(false);
		stub(wallet, "actsWithWifWithEncryption").returnValue(false);
		stub(wallet, "actsWithWif").returnValue(false);
