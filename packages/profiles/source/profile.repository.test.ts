...
describe("ProfileRepository", () => {
    ...
    it("should push, get, list and forget any given profiles", async () => {
        ...
        assert.is(subject.count(), 1);
        assert.is(subject.findById(john.id()) instanceof Profile);

        const jane = subject.create("Jane");

        assert.is(subject.count(), 2);
        assert.is(subject.findById(jane.id()) instanceof Profile);
        assert.is(subject.findByName(jane.name()) instanceof Profile);
        assert.is(subject.has(jane.id()), true);

        subject.forget(jane.id());

        assert.is(subject.count(), 1);
        assert.is(subject.has(jane.id()), false);
        assert.is(() => subject.findById(jane.id())).toThrow("No profile found for");
    });
    ...
    it("should get all profiles", async () => {
        ...
        assert.is(Object.keys(subject.all())).toHaveLength(2);
    });
    ...
    it("should get all values", async () => {
        ...
        assert.is(subject.values()).toHaveLength(2);
    });

    it("should forget all values", async () => {
        subject.create("Jane");

        assert.is(subject.values()).toHaveLength(1);

        subject.flush();

        assert.is(subject.values()).toHaveLength(0);
    });
    ...
    it("should fail to push a profile with a duplicate name", async () => {
        ...
        assert.is(() => subject.create("John")).toThrow("The profile [John] already exists.");
    });

    it("should fail to forget a profile that doesn't exist", async () => {
        assert.is(() => subject.forget("doesnotexist")).toThrow("No profile found for");
    });
...
it("should import ok", async () => {
    ...
    await assert.is(subject.import(data)).resolves.toBeInstanceOf(Profile);
});
...
it("should import ok with password", async () => {
    ...
    await assert.is(subject.import(data, "some pass")).resolves.toBeInstanceOf(Profile);
});
...
it("should restore", async () => {
    ...
    const profile = subject.create("John");
    await assert.is(subject.restore(profile)).toResolve();
});

it("should dump", async () => {
    const profile = subject.create("John");

    assert.is(subject.dump(profile)), "object");
});
