...
describeWithContext("NotificationService", { mnemonic: identity.mnemonic }, ({ it, assert, loader, beforeEach, nock }) => {
	...
	it("#markAsRead", async (context) => {
		...
		const notification = context.subject.transactions().findByTransactionId(context.notificationTransactionFixtures.data[1].id);
		context.subject.markAsRead(notification?.id);

		const notification2 = context.subject.transactions().findByTransactionId(context.notificationTransactionFixtures.data[2].id);
