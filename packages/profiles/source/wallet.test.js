...
test("should have a converted balance if it is a live wallet", async () => {
	...
	const live = stub(subject.network(), "isLive").mockReturnValue(true);
	const test = stub(subject.network(), "isTest").mockReturnValue(false);
...
test("should not have a converted balance if it is a test wallet", async () => {
	const live = stub(subject.network(), "isLive").mockReturnValue(false);
	const test = stub(subject.network(), "isTest").mockReturnValue(true);
...
test("should have a display name (knownName)", () => {
	const usernameSpy = stub(subject, "username").mockReturnValue(undefined);
...
test("should throw if a connection is tried to be established but no coin has been set", async () => {
	...
	stub(subject, "hasCoin").mockReturnValue(false);
