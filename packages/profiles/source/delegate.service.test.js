...
void describeWithContext("DelegateService", { mnemonic: identity.mnemonic }, async ({ beforeEach, it, assert, loader, nock, stub }) => {
	...
	it("should sync the delegates only one page", async (context) => {
		nock.cleanAll();
		nock.fake(/.+/).get("/api/delegates").reply(200, loader.json("test/fixtures/client/delegates-single-page.json"));
