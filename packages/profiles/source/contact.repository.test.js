...
void describeWithContext("ContactRepository", {
	...
	it("#create", (context) => {
		...
		assert.throws(() => context.subject.create(context.name, [context.addr]), `The contact [${context.name}] already exists.`);
	...
	it("#update with addresses", (context) => {
		...
		assert.throws(() => context.subject.update(contact.id(), { addresses: [] }), '"addresses" must contain at least 1 items');
