...
import { objectSchema, objectSchemaRequired } from './helpers/yup.js';
...
export class ProfileValidator implements IProfileValidator {
	public validate(data: IProfileData): IProfileData {
		return yup.object({
			...
			exchangeTransactions: objectSchemaRequired(
				yup.string().uuid(),
				yup.object({
					id: yup.string().required(),
			...
			notifications: objectSchemaRequired(
				yup.string().uuid(),
				yup.object({
					id: yup.string().required(),
					icon: yup.string(),
