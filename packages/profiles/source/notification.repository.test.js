...
describeWithContext("NotificationRepository", {
	...
	beforeEach((context) => {
		...
		context.subject = new NotificationRepository(new Profile({ avatar: "avatar", data: "", id: "uuid", name: "name" }));
	});

	it("#all", (context) => {
		assert.length(context.subject.keys(), 0);

		for (const n of context.stubNotifications)  {context.subject.push(n);}
	...
	it("#filterByType", (context) => {
		...
		for (const n of context.transactionNotifications)  {context.subject.push(n);}
		for (const n of context.releaseNotifications)  {context.subject.push(n);}
	...
	it("#findByTransactionId", (context) => {
		...
		assert.is(context.subject.findByTransactionId("1")?.meta?.transactionId, context.transactionNotifications[0]?.meta.transactionId);
	...
	it("#findByVersion", (context) => {
		...
		assert.is(context.subject.findByVersion("3.0.0")?.meta?.version, context.releaseNotifications[0]?.meta.version);
