...
describeWithContext("ProfileTransactionNotificationService", { mnemonic: identity.mnemonic }, ({ beforeEach, it, assert, loader, nock, stub }) => {
	beforeEach(async (context) => {
		...
		context.subject = new ProfileTransactionNotificationService(context.profile, context.notificationsRepository);
	...
	it("#forgetByRecipient", async (context) => {
		nock.fake(/.+/).get("/api/transactions").query(true).reply(200, context.notificationTransactionFixtures).persist();
	...
	it("#transactions", async (context) => {
		...
		assert.instance(context.subject.transaction(context.includedTransactionNotificationId), ExtendedConfirmedTransactionData);
	});
