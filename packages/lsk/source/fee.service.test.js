...
describe("FeeService", () => {
    describe("#all", () => {
        test("should succeed", async () => {
            ...
            assert.is(result).toContainAllKeys([
                "transfer",
                "secondSignature",
    ...
    describe("#calculate", () => {
        ...
        test("transfer", async () => {
            ...
            assert.is(slow.toHuman()), "number");
        assert.is(slow.toHuman(), 0.00141);
        assert.is(average.toHuman()), "number");
    assert.is(average.toHuman(), 0.00141);
    assert.is(fast.toHuman()), "number");
...
test("should throw error on unrecognized transaction type", async () => {
    await assert.is(subject.calculate({ moduleID: 10, assetID: 10, asset: {} })).rejects.toThrowError(
