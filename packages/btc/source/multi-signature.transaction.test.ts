...
describe("MultiSignatureTransaction", async ({ assert, beforeAll, each }) => {
	...
	each("isMultiSignatureRegistration", ({ dataset }) => {
	...
	each("isMultiSignatureRegistration for transfer", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
		assert.is(subject.isMultiSignatureRegistration(), dataset.expected);
	}, [
	...
	each("needsSignatures for musig registration", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
	...
	each("needsSignatures for transfer", ({ dataset }) => {
	...
	each("needsAllSignatures for musig registration", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
	...
	each("needsAllSignatures for transfer", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
		assert.is(subject.needsAllSignatures(), dataset.expected);
	}, [
		{ tx: unsignedNativeSegwitMusigTransferTx, expected: false },
	...
	each("isMultiSignatureReady for musig registration", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
	...
	each("isMultiSignatureReady for transfer", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
		assert.is(subject.isMultiSignatureReady(), dataset.expected);
	}, [
		{ tx: unsignedNativeSegwitMusigTransferTx, expected: false },
	...
	each("needsWalletSignature for musig registration", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
		assert.is(
	...
	each("needsWalletSignature for transfer", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
		assert.is(
			subject.needsWalletSignature(
				"Vpub5mtyU6Hx9xrx63Y3W4aGW1LuQkmwrq9xsQNgX7tDAM8DTHhE7vXMZ7Hue2FR8SMAGDW57fy76HFmN1jnckSmeX2cDMWVA1KViot6bLgJZuN",
	...
	each("needsFinalSignature for musig registration", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
	...
	each("remainingSignatureCount for musig registration", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
	...
	each("remainingSignatureCount for transfer", ({ dataset }) => {
		const subject = new PendingMultiSignatureTransaction(dataset.tx, bitcoin.networks.testnet);
		assert.is(subject.remainingSignatureCount(), dataset.expected);
	}, [
