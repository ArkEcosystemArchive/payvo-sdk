...
test("#toUNIX", () => {
	expect(subject.toUNIX()).toBe(1_577_836_800);
});

test("#valueOf", () => {
	expect(subject.valueOf()).toBe(1_577_836_800_000);
});
...
test("#isValid", () => {
	expect(subject.isValid()).toBeTrue();

	const invalidDate = DateTime.make("abc");

	expect(invalidDate.isValid()).toBeFalse();
});
