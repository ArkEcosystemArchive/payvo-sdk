...
describe("TransactionService", () => {
    describe("#transfer", () => {
        it("should verify", async () => {
            ...
            assert.is(Transactions.TransactionFactory.fromJson(result.data()).verify(), true);
        ...
        it("should add a signature if the sender public key is a multi-signature wallet", async () => {
            ...
            assert.is(result.data().signatures).toHaveLength(1);
        });
    });
    ...
    test("#estimateExpiration", async () => {
        ...
        await assert.is(subject.estimateExpiration()).resolves, "6795392");
});
});
